...
public class T5524经营摩天轮的最大利润 {
    public int minOperationsMaxProfit(int[] customers, int boardingCost, int runningCost) {
        int res = 0, bestBenefit = 0;
        int tempBenefit = 0, tempCustomes = 0;
        for (int i = 0; i < customers.length || tempCustomes > 0; i++) {
            if (i < customers.length)
                tempCustomes += customers[i];
            if (tempCustomes >= 4) {
                tempBenefit += (boardingCost << 2) - (runningCost);
                tempCustomes -= 4;
            } else {
                tempBenefit += (boardingCost * tempCustomes) - (runningCost);
                tempCustomes = 0;
            }
