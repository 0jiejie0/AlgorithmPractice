package main.problemAndSolving.leetcode_20241124WeekRankList;
...
//s 和 t 仅由小写英文字母组成。
//输入保证 s 和 t 互为字母异位词。
public class T100445重排子字符串以形成目标字符串 {
    //    @AssertExample(params = {"abcd", "cdab", "2"}, expectResult = "true")
...
//        LinkedList<String> list = new LinkedList<>();
        ...
        i = 0;
        while (t.length() >= i + l) {
            String substring = t.substring(i, l + i);
            if (map.containsKey(substring)) {
                Integer c = map.get(substring);
                if (c == 1) {
...
// 这里碰到了个坑，因为字符串达到5次方两级，substring耗时和要截取长度成正比，
// 如果仅仅对其中某一子串使用substring还好，但是如果截取长度长、次数多，就会超时